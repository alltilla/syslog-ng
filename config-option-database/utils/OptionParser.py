...
def _get_resolve_db():
    ...
    if not resolve_db:
        ...
        for f in Path(__file__).parents[2].rglob('*-parser.c'):
...
def _resolve_token(token, is_type=False):
    type_template = '<{}>'
...
def _resolve_tokens(tokens, is_type=False):
    resolved_tokens = []
    for token in tokens:
        resolved_tokens.append(_resolve_token(token, is_type))
...
def _resolve_option(context, driver, keyword, arguments, parents):
    ...
    arguments = _resolve_tokens(arguments, is_type=True)
