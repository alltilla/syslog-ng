...
static void
_load_data_from_persist(GroupingBy *self, GlobalConfig *cfg)
{
  GroupingByPersistData *persist_data = cfg_persist_config_fetch(cfg, grouping_by_format_persist_name(self)); // FETCH
  if (persist_data)
    {
...
grouping_by_init(LogPipe *s)
{
  ...
  _load_data_from_persist(self, cfg);
...
_store_data_in_persist(GroupingBy *self, GlobalConfig *cfg)
{
  ...
  cfg_persist_config_add(cfg, grouping_by_format_persist_name(self), persist_data,
                         (GDestroyNotify) _free_persist_data, FALSE); // ADD
