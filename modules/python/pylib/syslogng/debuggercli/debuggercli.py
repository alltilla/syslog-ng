...
class DebuggerCLI(object):
    ...
    def _setup_tflang_completers(self):
        self.tflang_completers['COMMAND'] = ChoiceCompleter(
            get_template_functions())
        self.tflang_completers['name_value_name'] = ChoiceCompleter(
            get_nv_registry())
        self.tflang_completers['value_pairs_scope'] = ChoiceCompleter(
            get_value_pairs_scopes())
        self.tflang_completers['OPT'] = ChoiceCompleter(
            TemplateFunctionLang.known_options)
        self.tflang_completers['template'] = LangBasedCompleter(
            parser=TemplateLang(), completers=self.template_completers)
    ...
    def _setup_debug_completers(self):
        self.debug_completers['COMMAND'] = ChoiceCompleter(
            get_debugger_commands())
        self.debug_completers['template'] = LangBasedCompleter(
            parser=TemplateLang(), completers=self.template_completers)
    ...
    def get_root_completer(self):
        ...
        return LangBasedCompleter(
            parser=DebugLang(), completers=self.debug_completers)
