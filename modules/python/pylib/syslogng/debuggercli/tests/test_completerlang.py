...
class CompleterLangTestCase(unittest.TestCase):
    def _get_expected_tokens(self, text):
        ...
        return self._parser.get_expected_tokens(
            text, drop_last_token=drop_last_token)

    def _assert_token_follows(self,
                              text,
                              tokens,
                              pos=None,
                              replaced_token=None):
        ...
        if replaced_token is not None:
            self.assertEqual(rtoken.value if rtoken is not None else '',
                             replaced_token)
...
class TestCompleterLang(CompleterLangTestCase):
    ...
    def test_all_tokens_are_found_that_would_match_rules_in_the_grammar(self):
        ...
        self._assert_token_follows(
            "foo bar", ["ARG"], pos=4, replaced_token="bar")
        self._assert_token_follows(
            "foo bar ", ["ARG"], pos=8, replaced_token="")
        self._assert_token_follows(
            "print ", ["template", "ARG"], pos=6, replaced_token="")
        self._assert_token_follows(
            "print $MSG", ["template", "ARG"], pos=6, replaced_token="$MSG")
        self._assert_token_follows(
            "print $MSG ", [], pos=11, replaced_token="")
