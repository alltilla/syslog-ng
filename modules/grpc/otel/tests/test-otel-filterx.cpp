...
Test(otel_filterx, resource_schema_url)
{
  FilterXObject *resource = (FilterXObject *) otel_resource_new(NULL);
  FilterXObject *foo = filterx_string_new("foo", -1);
  FilterXObject *number = filterx_integer_new(42);

  FilterXObject *schema_url;

  schema_url = filterx_object_getattr(resource, "schema_url");
  cr_assert_str_eq(filterx_string_get_value(schema_url, nullptr), "");
  filterx_object_unref(schema_url);

  cr_assert(filterx_object_setattr(resource, "schema_url", foo));
  schema_url = filterx_object_getattr(resource, "schema_url");
  cr_assert_str_eq(filterx_string_get_value(schema_url, nullptr), "foo");
  filterx_object_unref(schema_url);

  cr_assert_not(filterx_object_setattr(resource, "schema_url", number));
