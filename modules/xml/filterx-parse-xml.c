                                        ...
                                        guint64 *num_of_elems)
{
  ...
  if (filterx_object_is_type(existing_inner_obj, &FILTERX_TYPE_NAME(string)))
  ...
  else if (filterx_object_is_type(existing_inner_obj, &FILTERX_TYPE_NAME(list)))
    {
      /*
    ...
    {
      g_assert_not_reached(); // TODO: proper error handling
    }
                                               ...
                                               guint64 num_of_elems)
{
  ...
  msg_error("TODO: unexpected type");
  ...
  if (num_of_elems == 1)
    {
      ...
      if (!filterx_object_set_subscript(current_obj, attrs_obj_key, dict))
        {
          filterx_object_unref(dict); // TODO: error?
          dict = NULL;
        }
  ...
  if (filterx_object_is_type(existing_obj, &FILTERX_TYPE_NAME(dict)))
    {
      ...
      if (!filterx_object_set_subscript(current_obj, attrs_obj_key, new_list))
        {
          filterx_object_unref(new_list); // TODO: error?
          goto exit;
        }
                                      ...
                                      guint64 num_of_elems)
{
  ...
  if (!has_attrs)
    {
      _prepare_dict_for_attribute_collection_no_attrs(element_name, current_obj, num_of_elems);
      return NULL;
    }

  return _prepare_dict_for_attribute_collection_has_attrs(element_name, current_obj, num_of_elems);
}
                            ...
                            guint64 num_of_elems)
{
  FilterXObject *dict = _prepare_dict_for_attribute_collection(element_name, current_obj,
                                                             attribute_names, attribute_values, num_of_elems);
  if (!dict)
    return;
...
static FilterXObject *
_ensure_dict(GMarkupParseContext *context, GQueue *obj_stack)
{
  FilterXObject *dict = NULL;
  ...
  g_assert(filterx_object_is_type(current_obj, &FILTERX_TYPE_NAME(string)));

  /*
   * We turned out to be a node instead of a leaf.
   * Let's transform the empty string to an empty dict.
   */

  gsize len;
  filterx_string_get_value(current_obj, &len);
  if (len != 0)
    {
      // TODO: error?
      return NULL;
               ...
               gpointer user_data, GError **error)
{
  ...
  FilterXObject *current_obj = _ensure_dict(context, obj_stack);

  guint64 num_of_elems;
  FilterXObject *inner_obj = _prepare_inner_object_for_start_element(element_name, current_obj, &num_of_elems);
  g_queue_push_head(obj_stack, inner_obj);

  _collect_element_attributes(element_name, current_obj, attribute_names, attribute_values, num_of_elems);
}
