...
from driver_db import Block, Driver, Option
...
def __is_block(sentence: Tuple[str, ...]) -> bool:
    ...
    raise ParseError("Block is not closed: {}".format(sentence))
...
def __is_positional_option(sentence: Tuple[str, ...]) -> bool:
    if len(sentence) < 1:
        raise ParseError("Too short sentence: {}".format(sentence))
...
def __mark_n_symbols_as_processed(processed: Set[int], start_index: int, n: int) -> None:
    processed |= set(range(start_index, start_index + n))
