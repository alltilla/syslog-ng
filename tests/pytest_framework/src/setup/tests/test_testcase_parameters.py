...
def test_testcase_parameters(request):
    ...
    assert set(list(testcase_parameters)) == {
        "testcase_name", "dirs", "file_paths", "loglevel", "valgrind_usage"
    }
    assert set(list(testcase_parameters["dirs"])) == {
        "working_dir", "relative_working_dir"
    }
    assert set(list(testcase_parameters["file_paths"])) == {
        "report_file", "testcase_file"
    }
