...
class ConfigRenderer(object):
    ...
    def __render(self, re_create_config=None):
        ...
        if self.__syslog_ng_config["logpath_groups"]:
            self.__render_logpath_groups(
                self.__syslog_ng_config["logpath_groups"])

    def __render_version(self):
        self.__syslog_ng_config_content += "@version: {}\n".format(
            self.__syslog_ng_config["version"])
    ...
    def __render_global_options(self):
        ...
        for option_name, option_value in self.__syslog_ng_config[
    ...
    def __render_positional_options(self, driver_options, positional_options):
        for option_name, option_value in driver_options.items():
            if option_name in positional_options:
                if str(self.__instance_paths.get_working_dir()) not in str(
                        option_value):
                    driver_options[option_name] = Path(
                        self.__instance_paths.get_working_dir(), option_value)
    ...
    def __render_statement_groups(self):
        for statement_group in self.__syslog_ng_config["statement_groups"]:
            ...
            for statement in statement_group.statements:
                ...
                # driver options
                self.__render_positional_options(
                    statement.options, statement.positional_option_name)
                self.__render_driver_options(statement.options,
                                             statement.positional_option_name)
