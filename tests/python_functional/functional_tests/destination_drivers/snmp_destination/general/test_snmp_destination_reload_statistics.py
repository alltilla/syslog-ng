...
def test_snmp_dest_reload_stat(config, syslog_ng, syslog_ng_ctl, snmptrapd, snmp_test_params):
    ...
    assert snmp_destination.get_query() == {'written': message_counter, 'processed': message_counter, 'dropped': 0, 'queued': 0}
    ...
    for expected_trap in snmp_test_params.get_expected_basic_trap():
        assert received_traps.count(expected_trap) == message_counter
