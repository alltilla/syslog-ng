...
def test_proxy_protocol_with_multiple_clients(config, port_allocator, syslog_ng):
    host = "localhost"
    port = port_allocator()
    ...
    network_source = config.create_network_source(ip=host, port=port, transport="proxied-tcp", flags="no-parse")
    ...
    Loggen().start(host, port, inet=True, stream=True, read_file=loggen1_input_path, dont_parse=True, rate=1)
    Loggen().start(host, port, inet=True, stream=True, read_file=loggen2_input_path, dont_parse=True, rate=1)
